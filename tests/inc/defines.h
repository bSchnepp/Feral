...
{ \
	fprintf(stderr, \
		"In test %s of category %s: \n", TestName, TestCategory); \
	fprintf(stderr,  \
		"\033[1;31mFAILED TEST: %lld not equal to %lld\033[0m\n", x, y); \
	return TEST_STATUS_ERR; \
} else { \
	fprintf(stderr, \
		"In test %s of category %s: \n", TestName, TestCategory); \
	fprintf(stderr, \
		"\033[1;32mPASSED TEST: %lld is equal to %lld\033[0m\n", x, y);	\
}
...
{ \
	...
	fprintf(stderr, \
		"In test %s of category %s: \n", TestName, TestCategory); \
	fprintf(stderr, \
		"\033[1;32mPASSED TEST: %lld not equal to %lld\033[0m\n", x, y);	\
}

#define ASSERT_EQ_STR(x, y, TestName, TestCategory) \
TEST_INTERNAL_LengthOne = strlen(x); \
TEST_INTERNAL_LengthTwo = strlen(y); \
TEST_INTERNAL_Lesser = (TEST_INTERNAL_LengthOne < TEST_INTERNAL_LengthTwo) ? TEST_INTERNAL_LengthOne : TEST_INTERNAL_LengthTwo; \
if ((strncmp(x, y, TEST_INTERNAL_Lesser) != 0) || (TEST_INTERNAL_LengthOne != TEST_INTERNAL_LengthTwo)) \
{ \
	fprintf(stderr, \
		"In test %s of category %s: \n", TestName, TestCategory); \
	fprintf(stderr,  \
...
#define ASSSERT_TRUE(x, TestName, TestCategory) ASSERT_EQ_LLD(x, TRUE, TestName, TestCategory);
#define ASSERT_FALSE(x, TestName, TestCategory) ASSERT_EQ_LLD(x, FALSE, TestName, TestCategory);
